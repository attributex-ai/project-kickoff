...
description: Find the root cause of a failure instead of guessing at fixes. Use this skill whenever the verify gate goes red, a test fails unexpectedly, the build breaks, or the app won't boot during a project build. It runs a short structured process: reproduce, locate, understand, fix the cause (not the symptom), and confirm. This is the project-kickoff chain's recovery discipline for builds generated by the execution skill. Trigger the moment a build failure appears, before attempting any speculative fix.
---
...
- **Respect the loop ceiling.** If `project-kickoff:verification-before-completion` has hit its iteration cap, stop and report the root cause you found (or the point you got stuck at) rather than continuing to churn.
